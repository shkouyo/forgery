...
// rewriteWorkflowPayload returns a copy of task whose workflow payload has
// checkout steps pointed at forgejoURL via the with.github-server-url input,
// so actions/checkout clones straight from the owning Forgejo instance
// instead of through Forgery's proxy. The store's task is never mutated: the
...
func rewriteWorkflowPayload(task *v1.Task, forgejoURL string) *v1.Task {
	if task == nil || len(task.WorkflowPayload) == 0 || forgejoURL == "" {
		return task
	}
	rewritten, err := injectCheckoutServerURL(task.WorkflowPayload, forgejoURL)
	if err != nil || bytes.Equal(rewritten, task.WorkflowPayload) {
		return task
	}
