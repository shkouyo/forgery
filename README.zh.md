...
**Forgery** 是一个 Go 守护进程, 充当 Forgejo 与 GitHub Actions 之间的**双向 gRPC 代理**.
它对 Forgejo 实例来说就像是一个普通的 Forgejo Actions runner -- 它会注册, 轮询任务并报告
结果 -- 但实际并不在本地执行作业. 相反, 它将执行委托给 GitHub Actions Workflow,
真正的 `forgejo-runner` 在其中启动, 连接回 Forgery, 并运行作业.
日志和状态更新通过 Forgery 流回 Forgejo.
...
在 Forgery 工作目录创建 `.env` 文件 (或在 shell 中导出).
至少需要以下内容:
...
完整配置参考见 [Configuration](#configuration).
...
一个 TLS 终止反向代理. 示例见 [Reverse Proxy Setup](#reverse-proxy-setup).
...
|----------|----------|---------|-------------|
| `FORGEJO_URL` | Yes | -- | Forgejo 实例基础 URL (如 `https://forgejo.example.com`) |
| `FORGEJO_RUNNER_TOKEN` | Yes | -- | Forgejo runner 注册令牌 |
| `FORGEJO_RUNNER_NAME` | Yes | -- | 在 Forgejo runner 列表中显示的显示名称 |
| `FORGEJO_RUNNER_LABELS` | Yes | -- | 逗号分隔的标签及可选的容器镜像映射 (如 `ubuntu-latest:docker://node:20-bookworm,docker:docker://ghcr.io/catthehacker/ubuntu:act-latest`) |
| `GITHUB_TOKEN` | Yes | -- | 具有 `workflow` 权限的 GitHub 个人访问令牌 (建议使用细粒度, 仓库作用域的令牌) |
| `GITHUB_REPO` | Yes | -- | 存放 workflow 的仓库, 格式为 `owner/repo` |
| `GITHUB_WORKFLOW_ID` | Yes | -- | Workflow 文件名 (如 `forgery-runner.yml`) 或 workflow ID |
| `GITHUB_REF` | No | `main` | 触发 workflow 的分支 ref |
| `GITHUB_API_URL` | No | `https://api.github.com` | GitHub API 基础 URL (使用 GitHub Enterprise 时设置) |
| `LISTEN_ADDR` | No | `:8443` | 南向 gRPC 监听地址 |
| `PUBLIC_URL` | No | `https://<hostname>:8443` | Forgery 的公网可达 URL (供内部 runner 连接回 Forgery) |
| `DEFAULT_CONTAINER_IMAGE` | No | -- | 没有显式 `:docker://...` 映射的标签使用的默认容器镜像 |
...
| `PING_KEEPALIVE` | No | `true` | 在背压期间发送 Ping RPC 以保持 Forgejo 连接 |
...
| `TLS_INSECURE_SKIP_VERIFY` | No | `false` | 跳过北向连接的 TLS 证书验证 (仅用于开发环境) |
