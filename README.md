...
**Checkout and artifacts via Forgery** — the internal runner's `GITHUB_SERVER_URL`, `GITHUB_API_URL`, and artifact endpoints all point at Forgery's `public_url` (forgejo-runner derives them from `--url`). To keep git traffic out of the proxy path, Forgery rewrites the task payload it hands to the runner: `github-server-url` is injected into every `actions/checkout` step, and server-URL expressions — `${{ github.server_url }}`, `${{ github.api_url }}`, `${{ github.repository_url }}`, `${{ env.GITHUB_SERVER_URL }}`, `${{ env.GITHUB_API_URL }}`, `${{ env.FORGEJO_SERVER_URL }}`, `${{ env.FORGEJO_API_URL }}` — are normalized to the owning instance's `forgejo_url` literal, so checkout clones, API calls, and container-registry pushes (`docker/login-action`'s `registry:` input) connect straight to Forgejo; the remaining `/api/v1/repos/...` default-branch query and `/api/actions_pipeline/...` artifact endpoints are reverse-proxied by the task token in the request's `Authorization` header — a token Forgejo itself issued, so the upstream instance authenticates it directly. Unknown tokens get `401`; with exactly one configured instance, unregistered tokens fall back to it. A cross-repository checkout without an explicit `ref` needs one: Forgejo's API v3 cannot serve checkout's default-branch lookup for another repository. Known boundary: `$GITHUB_SERVER_URL` read from the shell environment inside a `run:` step cannot be rewritten — the runner hard-codes it from `--url`.
